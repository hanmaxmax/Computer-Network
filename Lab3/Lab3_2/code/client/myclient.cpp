	...
	while (1)
	{
		...
		if (recvByte > 0)
		{
			...
			if (recvMsg.check())
			{
				...
				//打印窗口情况
				cout << "【当前窗口情况】 窗口总大小：" << N << "，已发送但未收到ACK：" << nextseqnum - base
...
void clientSendFunction_GBN2(string filename, SOCKADDR_IN serverAddr, SOCKET clientSocket)
{
	int startTime = clock();
	...
	{
		//rdt_send(data)
		if (nextseqnum < base + N && base < batchNum + 2)
		{
			//make_pkt
			Message sendMsg;
			if (nextseqnum == 0)
			...
			nextseqnum++;
			//打印窗口情况
			cout << "【当前窗口情况】 窗口总大小：" << N << "，已发送但未收到ACK：" << messageBuffer.size()
				<< "，尚未发送：" << N - messageBuffer.size() << "\n";
