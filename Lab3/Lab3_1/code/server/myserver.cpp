...
const int RouterPORT = 30000; //服务器端口号
	...
	while (1)
	{
		...
		if (recvByte > 0)
		{
			...
			else if (recvMsg.check() && (recvMsg.SeqNum != global_seq + 1))
			{
				...
				replyMessage.setCheck();
				sendto(serverSocket, (char*)&replyMessage, sizeof(replyMessage), 0, (sockaddr*)&clientAddr, sizeof(SOCKADDR_IN));
				cout << "【重复接收报文段】server收到 Seq = " << recvMsg.SeqNum << "的报文段，并发送 Ack = " << replyMessage.AckNum << " 的回复报文段" << endl;
			}
		}
		else if (recvByte == 0)
		{
...
void serverRecvFunction(SOCKET serverSocket, SOCKADDR_IN clientAddr)
{
	...
	while (1)
	{
		...
		{
			//成功收到消息
			if (nameMessage.check() && (nameMessage.SeqNum == global_seq + 1))
			{
				global_seq++;
			...
			//如果seq<期待值，则传来了重复的报文：丢弃重复报文，重传该报文的回复ACK
			//（注：这种情况是由于上一个回复ACK报文段丢失，导致发送方超时重传所致，传了重复的报文段）
			else if (nameMessage.check() && (nameMessage.SeqNum != global_seq + 1))
			{
				//回复ACK
				Message replyMessage;
				replyMessage.SrcPort = ServerPORT;
				...
				replyMessage.setCheck();
				sendto(serverSocket, (char*)&replyMessage, sizeof(replyMessage), 0, (sockaddr*)&clientAddr, sizeof(SOCKADDR_IN));
				cout << "【重复接收报文段】server收到 Seq = " << nameMessage.SeqNum << "的报文段，并发送 Ack = " << replyMessage.AckNum << " 的回复报文段" << endl;
			}
		}
	}
	...
	cout << "开始传输数据段，共 " << batchNum << " 个最大装载报文段\n" << endl;
